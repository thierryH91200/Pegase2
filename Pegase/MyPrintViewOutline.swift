...
class MyPrintViewOutline: NSView
{
    ...
    init(tableView tableToPrint: NSOutlineView?, andHeader header: String?) {
        ...
        self.header = header!
        listFont = NSFont(name: "Helvetica", size: 8.0)
        ...
        for indexPage in 0..<( lastEntryOfPage - firstEntryOfPage) {
            ...
            for column in columns {
                ...
                if let tableCellView = tableToPrint?.view(atColumn: numCol, row: row, makeIfNecessary: true) as? KSHeaderCellView
                {
                    ...
                    attributes[.foregroundColor] = (tableCellView.textField?.textColor) ?? .black
                        
                        let rect = NSRect(
                            x: leftMargin + horizontalOffset + offsetX,
                            y: topMargin +  rowHeight * CGFloat(indexPage + 1),
                            width: widthQuotient * columnWidth - 2,
                            height: rowHeight)
                        
                        horizontalOffset += widthQuotient * columnWidth
...
//    func drawData(tableCellView: NSTableCellView , columnWidth: CGFloat, horizontalOffset: CGFloat) {
//
//        var horizontalOffset = horizontalOffset
//        let valueAsStr = (tableCellView.textField?.stringValue)!
    ...
    func drawColumnTitle() {
        ...
        for column in columns {
            ...
            let headerRect = NSRect(
                x: self.leftMargin + offsetX,
                y: self.topMargin,
                width: self.widthQuotient * columnWidth,
                height: self.rowHeight)
            
            offsetX += widthQuotient * columnWidth
...
//            print(columnTitle, columnWidth,  column.width, headerRect)
    ...
    func drawVerticalGrids() {
        ...
        for column in columns {
            ...
            print(fromPoint, toPoint, offsetX)
